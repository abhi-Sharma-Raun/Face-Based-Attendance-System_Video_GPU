...
async function startVerificationScan() {
    ...
    if (!name || !rollNum || !email) {
        alert("Please enter your name, roll number, and email before turning on the webcam scan.");
        return;
    }
...
async function submitRegistration() {
    ...
    if (!name || !rollNum || !email) {
        alert("Please provide name, roll number, and email details.");
        return;
    }
    ...
    formData.append('roll_num', rollNum);
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert(`Student ${result.name} and assigned roll_number: ${result.roll_num} is registered successfully`);
            resetScanState();
...
async function runAttendanceAnalysis() {
    ...
    if (!className) {
        alert("Please specify the target Class Name prior to executing analysis algorithms.");
        return;
    }
    if (!state.attendanceFile) {
        alert("Please select a valid classroom video stream first.");
        return;
    }
